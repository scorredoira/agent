...
func (lp *LazyProvider) ensureActiveProvider(ctx context.Context) bool {
	...
	for i, provider := range lp.providers {
		// Test availability with short timeout
		testCtx, cancel := context.WithTimeout(ctx, 2*time.Second)
		if provider.IsAvailable(testCtx) {
			lp.activeProvider = provider
			lp.current = i
			cancel()
			// Log successful provider selection (debug only)
			if i > 0 && os.Getenv("LLM_DEBUG") != "" {
				fmt.Fprintf(os.Stderr, "LLM Fallback: Using %s (provider #%d) - Previous providers unavailable\n", provider.GetName(), i+1)
			}
			return true
		}
		cancel()
		// Log failed attempt (debug only)
		if os.Getenv("LLM_DEBUG") != "" {
			fmt.Fprintf(os.Stderr, "Provider %s unavailable, trying next...\n", provider.GetName())
		}
	}
